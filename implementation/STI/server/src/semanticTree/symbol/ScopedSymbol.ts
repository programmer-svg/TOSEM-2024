...
export class ScopedSymbol extends BaseSymbol implements IScopedSymbol {
     ...
     *          or any of the parent scopes (conditionally).
     */
    public findInSymbolTable(name: string, localOnly = false): BaseSymbol | undefined {
        for (const child of this.#children) {
            if (child.name === name) {
                return child;
            }
        }
        // Nothing found locally. Let the child continue.
        if (!localOnly) {
            for (const child of this.#children) {
                if (child.name.charAt(0) === '@') {
                    const sym = child.resolveSync(name,false);
                    if (sym){
                        return sym;
                    }
                }
